...
void Dashboard::read(){
    ...
    speedCommand = readAndScaleSpeedPot(SPEED_POT_PIN);
    //Serial.print("Speed Pot");Serial.println(analogRead(SPEED_POT_PIN));
...
// Read an analogPin (value between 0-1023) and return a value between -100,100 with 10% safety margin
int8_t Dashboard::readAndScaleSpeedPot(int PintoRead){
    int8_t temp = ( ( ((float)analogRead(PintoRead) - 415)/(175) ) -1 ) * 110 ;
    return max(-100, min(temp, 100));
}
